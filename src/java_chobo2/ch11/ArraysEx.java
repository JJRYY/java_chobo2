...
//		arrayToBinarySearch();
		...
		// Student 클래스에 implements Comparable<Student> 추가하여 기본적으로 정렬이 가능해짐 (현재 기본 학번순 정렬)
		Arrays.sort(stdArr); 
		prnStudent(stdArr);
	...
	public static void arrayToBinarySearch() {
		...
		// 이분검색은 정렬되어 있어야 한다.
		// parallel로 시작하는 메서드들은 보다 빠른 검색을 얻기 위해 여러 쓰레드가 작업을 나누어 처리하도록 한다.
//		Arrays.sort(arr);
...
//		arList.add(10);	// 배열의 크기가 정해져 있기때문에 추가 불가능 (UnsupportedOperationException)
//		System.out.println(arList);
		ArrayList<Integer> al = new ArrayList<Integer>(arList); // ArrayList로 변경하여 추가
		al.add(10);
